...
// A signature image and the printed name/designation below it must share
// the exact same horizontal box, or the signature visibly floats off to
// one side instead of sitting directly above the name. Both use the same
// x/width here, centered, with a signature line for a proper "real
// document" look instead of a bare floating image.
function drawSignatureBlock(doc, { x, width, y, imageBuffer, name, designation }) {
  ...
  if (imageBuffer) {
    try {
      ...
      // Invalid/unsupported image data should never block PDF generation.
    }
  }
  doc.moveTo(x + width * 0.15, boxBottom).lineTo(x + width * 0.85, boxBottom).strokeColor("#c7cee0").stroke();
  doc.font("Helvetica-Bold").fontSize(9).fillColor("#16213a").text(name || "", x, boxBottom + 6, { width, align: "center" });
  doc.font("Helvetica").fontSize(7.5).fillColor("#7a8398").text(designation || "Authorised Signatory", x, boxBottom + 19, { width, align: "center" });
}
...
async function generateInvoicePdf(invoice) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const renderHeader = () => drawHeader(doc, "PROFORMA INVOICE", "Invoice / receipt", biz);
    renderHeader();
    ...
    finalizeFooters(doc, "This is a system-generated proforma invoice from Kuwarji Travels.");
    doc.end();
  });
}
