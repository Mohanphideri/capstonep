...
function drawHeader(doc, title, subtitle, business = {}) {
  ...
  try {
    doc.image(BRAND_LOGO_PATH, 24, 17, { fit: [205, 78], align: "left", valign: "center" });
  } catch {
...
async function generateInvoicePdf(invoice) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // "Bill to" and "Line items" are positioned relative to the actual
    // height of the block above them (not a fixed offset) — the business
    // block above can be 1-4 lines depending on which of address/phone/
    // email/GSTIN are filled in, and a fixed offset here collided with
    // that block whenever every field was present (the normal case for a
    // real, fully-configured business profile — not an edge case).
    const billToY = by + 24;
    sectionTitle(doc, "Bill to", billToY);
    doc.font("Helvetica-Bold").fontSize(10).fillColor("#16213a").text(cust.name || "-", 46, billToY + 25);
    doc.font("Helvetica").fontSize(8.5).fillColor("#4c5876");
    let cy = billToY + 41;
    [cust.phone ? `Mobile: ${cust.phone}` : null, cust.email, cust.address].filter(Boolean).forEach((line) => { doc.text(line, 46, cy); cy += 14; });

    const lineItemsY = cy + 24;
    sectionTitle(doc, "Line items", lineItemsY);
    const tableTop = lineItemsY + 27;
    ...
    // "Balance due" (the summary column's last row) sits at summaryY + 132.
    // Terms must clear that, not just summaryY itself — a fixed +175/cap-675
    // offset from summaryY alone assumed summaryY would always land near
    // its 500px floor, and silently overlapped the summary box on any
    // invoice where line items or business/customer details pushed
    // summaryY down (again, the normal case for a real invoice, not an
    // edge case).
    const summaryBottom = summaryY + 132;
    if (invoice.terms) {
      const termsY = Math.min(summaryBottom + 40, 700);
    ...
    if (signatureBuffer) {
      const signatureY = Math.min(summaryBottom + 60, 710);
      sectionTitle(doc, "Authorised signatory", signatureY);
      try {
