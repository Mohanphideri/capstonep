...
router.post("/fleet-gallery", async (req, res) => {
  try {
    ...
    const item = { id: randomUUID(), imageUrl: stored.url, imageKey: stored.key, altText: parsed.altText || "Kuwarji Travels fleet" };
    settings.fleetGallery = [...(settings.fleetGallery || []), item];
    await settings.save();
    return res.json({ success: true, item: { id: item.id, imageUrl: item.imageUrl, altText: item.altText } });
  } catch (err) {
