...
function serialize(s) {
  return {
    ...
    homepage: {
      heroTitle: s.homepage?.heroTitle || "Travel Comfortably.",
      heroAccent: s.homepage?.heroAccent || "Travel With Confidence.",
      heroBody: s.homepage?.heroBody || "Reliable buses, cars and tempo travellers for local, outstation and group travel.",
      heroImageUrl: s.homepage?.heroImageUrl || null,
    },
    banner: s.banner ? {
      id: s.banner.id,
...
const updateSchema = z.object({
  ...
  homepage: z.object({ heroTitle: z.string().trim().max(200).optional(), heroAccent: z.string().trim().max(200).optional(), heroBody: z.string().trim().max(600).optional(), heroImageUrl: z.string().trim().max(1000).nullable().optional() }).optional(),
...
router.post("/homepage-hero", async (req,res)=>{ try { const {parsed,buffer,validated}=await decodeImage(req.body); await connectToDatabase(); await ensureSiteSettingsSeed(); const settings=await SiteSetting.findOne({key:SETTINGS_KEY}); const storage=getStorageProvider(); const stored=await storage.save({buffer,extension:validated.extension,folder:"homepage"}); const oldKey=settings.homepage?.heroImageKey; settings.homepage={...(settings.homepage?.toObject?settings.homepage.toObject():settings.homepage||{}),heroImageUrl:stored.url,heroImageKey:stored.key}; await settings.save(); if(oldKey&&oldKey!==stored.key)await storage.delete(oldKey); await recordAuditLog({req,action:"HOMEPAGE_HERO_UPLOADED",entityType:"SiteSetting",entityId:settings._id,metadata:{filename:parsed.filename}}); return res.json({success:true,imageUrl:settings.homepage.heroImageUrl,settings:serialize(settings)}); }catch(err){console.error("homepage hero upload error",err);return res.status(400).json({success:false,error:err.message||"Failed to upload homepage hero image."});} });
router.delete("/homepage-hero", async (req,res)=>{ try { await connectToDatabase(); await ensureSiteSettingsSeed(); const settings=await SiteSetting.findOne({key:SETTINGS_KEY}); const oldKey=settings.homepage?.heroImageKey; settings.homepage={...(settings.homepage?.toObject?settings.homepage.toObject():settings.homepage||{}),heroImageUrl:null,heroImageKey:null}; await settings.save(); if(oldKey)await getStorageProvider().delete(oldKey); return res.json({success:true}); }catch(err){return res.status(500).json({success:false,error:"Failed to remove homepage hero image."});} });
