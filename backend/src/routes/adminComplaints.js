...
const { createComplaintNotification } = require("../lib/notify");
...
router.patch("/:ticketId/status", async (req, res) => {
  try {
    ...
    createComplaintNotification("STATUS_UPDATED", complaint);
    res.json({ success: true, complaint: serialize(complaint.toObject()) });
  } catch (err) {
...
router.post("/:ticketId/messages", async (req, res) => {
  try {
    ...
    const complaint = await findComplaint(req.params.ticketId);
    if (!complaint) return res.status(404).json({ success: false, error: "Issue not found." });
    if (complaint.status === "CLOSED") return res.status(400).json({ success: false, error: "This issue is closed. No further messages can be sent." });
    ...
    await complaint.save();
    createComplaintNotification("MESSAGE_ADDED", complaint);
    res.json({ success: true, complaint: serialize(complaint.toObject()) });
  } catch (err) {
