...
// alongside the stateless JWT cookie that authenticates the user. Writing
// this row is still best-effort — never blocks or fails the login
// response — but it's no longer just an audit log: middleware/requireAuth.js
// checks this row's `revokedAt` on every request (see
// lib/sessionRevocation.js), so logout below actually revokes the token.
