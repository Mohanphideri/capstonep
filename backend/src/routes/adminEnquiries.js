...
const { createEnquiryNotification } = require("../lib/notify");
...
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    createEnquiryNotification("STATUS_UPDATED", enquiry);
...
router.post("/:id/notes", async (req, res) => {
  try {
    ...
    // Only a staff reply is customer-facing news — a customer's own note
    // (if this endpoint is ever reused for that) shouldn't notify itself.
    if (req.session.role !== "customer") {
      createEnquiryNotification("NOTE_ADDED", enquiry);
    }
