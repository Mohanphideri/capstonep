...
const { createReviewNotification } = require("../lib/notify");
...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    createReviewNotification("SUBMITTED", review);
    ...
    if (err?.code === 11000 && err?.keyPattern?.bookingId) {
      return res.status(409).json({ success: false, error: "You have already reviewed this completed booking." });
    }
...
router.get("/mine", requireAuth, async (req, res) => {
  try {
    ...
    const reviews = await Review.find({ $or: ownership })
      .select("bookingId rating text status featured createdAt customerName state district")
      .populate("bookingId", "bookingId")
      .lean();
    return res.json({ success: true, reviews: reviews.map((r) => ({ bookingId: r.bookingId?.bookingId || r.bookingId?._id?.toString() || null, rating: r.rating, text: r.text, status: r.status, featured: r.featured, createdAt: r.createdAt, state: r.state, district: r.district })) });
  } catch (err) {
