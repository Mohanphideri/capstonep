...
const { createBookingNotification } = require("../lib/notify");
...
router.post("/", async (req, res) => {
  try {
    ...
    // A DRAFT booking isn't confirmed yet — don't tell the customer it is.
    if (booking.status !== "DRAFT") {
      createBookingNotification(booking.status === "CANCELLED" ? "CANCELLED" : "CONFIRMED", booking);
    }
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    const previousAmountReceived = Number(booking.pricing?.amountReceived || 0);
    if (d.pricing) {
    ...
    if (previousStatus !== booking.status && ["CANCELLED", "CONFIRMED", "IN_PROGRESS", "COMPLETED"].includes(booking.status)) {
      createBookingNotification(booking.status, booking);
    }
    if (Number(booking.pricing?.amountReceived || 0) > previousAmountReceived) {
      createBookingNotification("ADVANCE_RECEIVED", booking);
    }
...
router.post("/:id/cancel", async (req, res) => {
  try {
    ...
    createBookingNotification("CANCELLED", booking);
