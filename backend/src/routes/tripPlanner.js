...
// Each call here can hit the paid/rate-limited Groq API — cap abuse so a
// single visitor (or bot) can't run up usage or exhaust the Groq quota.
// Logged-in users get a higher ceiling than anonymous guests since
// they're identifiable and this is also where the customer-portal Trip
// Planner page (full, untruncated chat) lives.
const guestChatRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, max: 3 });
const authChatRateLimit = createRateLimiter({ windowMs: 15 * 60 * 1000, max: 20 });

function chatRateLimit(req, res, next) {
  const limiter = req.session ? authChatRateLimit : guestChatRateLimit;
...
// Groq-powered conversational trip assistant — this is the only trip-planning
// surface on the site now (the old multi-step Trip Maker wizard has been
// removed). Ask AI on the homepage talks to this endpoint.
