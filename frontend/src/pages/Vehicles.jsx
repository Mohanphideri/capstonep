...
export default function Vehicles({ embedded = false }) {
  ...
  const content = (
    ...
    <main className={embedded ? "vehicles-dashboard-content" : "container vehicles-page"}>
      ...
      {lightbox&&<div className="vehicle-lightbox" onClick={()=>setLightbox(null)}><div className="vehicle-lightbox-inner" onClick={e=>e.stopPropagation()}><button type="button" className="vehicle-lightbox-close" onClick={()=>setLightbox(null)}>×</button><img src={lightbox.photos[lightbox.index]} alt={`${lightbox.name} selected`} /><div className="vehicle-lightbox-controls"><button type="button" onClick={()=>setLightbox(x=>({...x,index:(x.index-1+x.photos.length)%x.photos.length}))}>←</button><span>{lightbox.index+1} / {lightbox.photos.length}</span><button type="button" onClick={()=>setLightbox(x=>({...x,index:(x.index+1)%x.photos.length}))}>→</button></div></div></div>}
