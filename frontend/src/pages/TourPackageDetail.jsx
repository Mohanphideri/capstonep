...
export default function TourPackageDetail({ embedded = false }) {
  ...
  const content = <main className="tour-detail"><Link to={embedded ? "/dashboard/tour-packages" : "/tour-packages"} className="tour-back">← All tour packages</Link><div className="tour-detail-card"><div className="tour-detail-image" style={pkg.imageUrl ? { backgroundImage: `url(${pkg.imageUrl})` } : undefined}></div><div className="tour-detail-copy"><span className="eyebrow">{pkg.destination} · {pkg.durationDays} days</span><h1>{pkg.title}</h1><p>{pkg.description}</p><button className="btn btn-primary" onClick={() => setOpen(true)}>Enquire for this package</button></div></div><div className="tour-detail-sections"><section><h2>Itinerary</h2><ol>{(pkg.itinerary || []).map((x,i)=><li key={i}>{x}</li>)}</ol></section><section><h2>Inclusions</h2><ul>{(pkg.inclusions || []).map((x,i)=><li key={i}>{x}</li>)}</ul><h2>Exclusions</h2><ul>{(pkg.exclusions || []).map((x,i)=><li key={i}>{x}</li>)}</ul></section></div></main>;
  return embedded ? <ConsumerLayout title="Tour Package" lead="Review the itinerary and send an enquiry for this journey.">{content}{open && <EnquiryDrawer open onClose={() => setOpen(false)} packageId={pkg.id} packageTitle={pkg.title} mode="tour" />}</ConsumerLayout> : <div className="page"><Navbar />{content}{open && <EnquiryDrawer open onClose={() => setOpen(false)} packageId={pkg.id} packageTitle={pkg.title} mode="tour" />}<Footer /></div>;
}
