      ...
      <div className="booking-detail-main">
        ...
        {!loading && booking && (
          <>
            ...
            {booking.status !== "CANCELLED" && <BookingTimeline booking={booking} />}
...
function journeyDurationLabel(journey) {
  if (!journey?.journeyStart) return null;
  if (!journey.journeyEnd) return "1 day";
  const start = new Date(journey.journeyStart);
  const end = new Date(journey.journeyEnd);
  const days = Math.max(1, Math.round((end - start) / 86400000) + 1);
  return `${days} day${days > 1 ? "s" : ""}`;
}

// Five fixed stages mirroring the booking's real lifecycle (see
// models/Booking.js: DRAFT -> CONFIRMED -> IN_PROGRESS -> COMPLETED, plus
// pricing.amountReceived for the advance-payment stage). Each stage is
// "done" (checked), the single "active" stage (filled), or still ahead.
function computeTimelineStages(booking) {
  const hasAdvance = Number(booking?.pricing?.amountReceived || 0) > 0;
  const labels = [
    "Enquiry received",
    "Booking confirmed",
    "Advance received",
    booking.status === "IN_PROGRESS" ? "Journey in progress" : "Upcoming journey",
    "Journey completed",
  ];
  ...
  return labels.map((label, i) => ({
    label,
    state: i < doneCount ? "done" : i === activeIndex ? "active" : "upcoming",
  }));
}

function BookingTimeline({ booking }) {
  const stages = computeTimelineStages(booking);
  return (
    <section className="ticket booking-timeline-section">
      ...
      <ol className="booking-timeline">
        {stages.map((stage, i) => (
          <li key={stage.label} className={`booking-timeline-step is-${stage.state}`}>
            <span className="booking-timeline-marker" aria-hidden="true">
              {stage.state === "done" ? <Icon name="check" size={12} /> : null}
            </span>
            <span className="booking-timeline-label">{stage.label}</span>
          </li>
        ))}
      </ol>
      <div className="booking-timeline-journey">
        <p className="eyebrow-muted">Journey</p>
        <p className="booking-timeline-route">
          {booking.journey.pickup} <Icon name="arrowRight" size={13} /> {booking.journey.destination}
        </p>
        <p className="booking-timeline-meta">
          {formatDate(booking.journey.journeyStart)}
          {formatPickupTime(booking.journey.pickupTime) ? ` · ${formatPickupTime(booking.journey.pickupTime)}` : ""}
          {" · "}
          {journeyDurationLabel(booking.journey)}
          {" · "}
          {booking.journey.passengers} passenger{Number(booking.journey.passengers) === 1 ? "" : "s"}
        </p>
      </div>
    </section>
  );
