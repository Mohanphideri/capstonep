...
export function FAQPage() {
  const [faqs,setFaqs]=useState([]); const [loading,setLoading]=useState(true); const [activeCategory,setActiveCategory]=useState("ALL");
  ...
  const items=(faqs.length?faqs:FALLBACK_FAQS.map(([question,answer])=>({question,answer,category:"Journey"}))).filter(f=>activeCategory==="ALL"||!activeCategory||f.category===activeCategory);
  return <div className="page"><SiteBanner /><Navbar /><main className="public-page faq-reference-page">
    <section className="container faq-reference-hero"><div className="faq-question-mark" aria-hidden="true">?</div><div><p className="eyebrow">Help centre</p><h1 className="public-title">Frequently Asked Questions</h1></div></section>
    <section className="container faq-reference-layout">
      <aside className="faq-topic-nav" aria-label="FAQ categories">{categories.map((x,i)=><button type="button" key={x} className={((activeCategory==="ALL"&&i===0)||activeCategory===x)?"active":""} onClick={()=>setActiveCategory(x)}>{x==="ALL"?"All questions":x}</button>)}</aside>
      <div className="faq-reference-list">{loading?<p className="public-state">Loading FAQs…</p>:items.length?items.map((item,i)=><details className="faq-reference-item" key={`${item.question}-${i}`} open={i===0}><summary><span>{item.question}</span><b aria-hidden="true">{i===0?"−":"+"}</b></summary><div className="faq-reference-answer">{item.answer}</div></details>):<div className="public-empty">No FAQs are available in this category yet.</div>}</div>
    </section>
  </main><Footer /></div>;
}
