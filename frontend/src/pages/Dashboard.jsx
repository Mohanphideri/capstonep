...
export default function Dashboard(){
  ...
  const [enquiries,setEnquiries]=useState([]); const [reviews,setReviews]=useState([]); const [publicReviews,setPublicReviews]=useState([]);
  useEffect(()=>{
    ...
    Promise.all([
      ...
      apiFetch("/api/reviews/featured").then(r=>r.ok ? r.data : null),
  ...
  if(!user)return null;
  const displayName=user.name||"Traveller"; const firstName=displayName.split(" ")[0]; const nextTrip=upcoming[0];
  return <ConsumerLayout title="Dashboard">
    <section className="customer-welcome-banner" data-tour-target="dashboard"><div><h2>Welcome back, {displayName}!</h2><p>Plan your next journey with us.</p></div><div className="customer-skyline" aria-hidden="true"><span className="city c1"/><span className="city c2"/><span className="city c3"/><span className="city c4"/><span className="bus-art">▰</span></div></section>
    ...
    <section className="customer-panel customer-upcoming-panel">
      ...
      {loadingBookings?<div className="customer-loading">Loading your bookings…</div>:nextTrip?<Link className="customer-booking-card" to={`/dashboard/bookings/${nextTrip.bookingId}`}><div className="customer-booking-vehicle"><Icon name="truck" size={38}/></div><div className="customer-booking-main"><div className="customer-booking-title-row"><strong>{nextTrip.vehicles?.map(v=>v?.name).filter(Boolean).join(", ")||"Upcoming Journey"}</strong><span className="customer-status">{String(nextTrip.status||"CONFIRMED").replace(/_/g," ")}</span></div><div className="customer-booking-route">{nextTrip.journey?.pickup||"Pickup"} <Icon name="arrowRight" size={15}/><b>{nextTrip.journey?.destination||"Destination"}</b></div><div className="customer-booking-meta"><span><Icon name="calendar" size={13}/>{formatDate(nextTrip.journey?.journeyStart)}</span><span>Booking ID <b>{nextTrip.bookingId||"—"}</b></span><span>Amount <b>{money(nextTrip.amount||nextTrip.totalAmount||0)}</b></span></div></div></Link>:<div className="customer-empty">No upcoming booking. <Link to="/dashboard/vehicles">Search vehicles</Link> to plan your next journey.</div>}
    </section>
    ...
    <section className="customer-quick-grid">
      <Link to="/trip-planner" className="customer-quick-card"><span className="quick-violet"><Icon name="calendar"/></span><strong>Trip Planner</strong><small>Plan your route and vehicle</small></Link>
      ...
      <Link to="/dashboard/tour-packages" className="customer-quick-card"><span className="quick-green"><Icon name="grid"/></span><strong>Tour Packages</strong><small></small></Link>
