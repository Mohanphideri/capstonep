...
export default function AdminSettings({ section = "business" }) {
  ...
  const [uploading, setUploading] = useState({}); const [messages, setMessages] = useState({});
  const load = useCallback(async () => { setLoading(true); const settingsResult = await apiFetch("/api/admin/settings"); if (settingsResult.ok && settingsResult.data?.success) setForm(toForm(settingsResult.data.settings)); else setError(settingsResult.data?.error || "Failed to load settings."); setLoading(false); }, []);
