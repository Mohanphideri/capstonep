...
const emptyForm = { customerName: "", customerPhone: "", customerEmail: "", rating: 5, text: "", vehicleId: "", status: "APPROVED", featured: true };
...
export default function AdminReviews() {
  ...
  async function load() {
    setLoading(true);
    const [{ ok, data }, vehicleResult] = await Promise.all([
          ...
          {reviews.map((review) => (
            <article className="admin-review-card" key={review.id}>
              <div className="admin-review-head">
                <div><span className="admin-review-id">{review.adminCreated ? "ADMIN REVIEW" : review.bookingId}</span><h2>{review.customer}</h2><p>{review.phone}{review.email ? ` · ${review.email}` : ""}</p></div>
