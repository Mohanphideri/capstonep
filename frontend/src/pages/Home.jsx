...
function scrollCards(scrollerRef, direction) {
...
export default function Home() {
  ...
  const fleetScrollerRef = useRef(null);
  const packageScrollerRef = useRef(null);
  ...
  const landingVehicles = useMemo(() => {
    ...
    return vehicles.filter((vehicle) => vehicle.photos?.length).slice(0, 8);
  }, [vehicles]);
  ...
  return (
    <div className="page home-page">
      ...
      <section className="home-section home-fleet" id="vehicles"><div className="container">
        ...
        {!loading && landingVehicles.length > 0 && <div className="home-horizontal-shell">
          <div className="home-horizontal-controls" aria-label="Fleet carousel controls"><button type="button" onClick={() => scrollCards(fleetScrollerRef, -1)} aria-label="Scroll fleet left">←</button><button type="button" onClick={() => scrollCards(fleetScrollerRef, 1)} aria-label="Scroll fleet right">→</button></div>
          <div className="home-fleet-grid home-horizontal-track" ref={fleetScrollerRef} tabIndex="0">
      ...
      {packages.length > 0 && <section className="home-section home-packages" id="tour-packages"><div className="container">
        <Reveal><div className="home-section-heading"><div><span className="home-eyebrow">TOUR PACKAGES</span><h2>Tour Packages</h2><p></p></div><Link to="/tour-packages" className="home-text-link">View All Packages →</Link></div></Reveal>
        <div className="home-horizontal-shell">
          <div className="home-horizontal-controls" aria-label="Tour package carousel controls"><button type="button" onClick={() => scrollCards(packageScrollerRef, -1)} aria-label="Scroll tour packages left">←</button><button type="button" onClick={() => scrollCards(packageScrollerRef, 1)} aria-label="Scroll tour packages right">→</button></div>
          <div className="home-package-grid home-horizontal-track" ref={packageScrollerRef} tabIndex="0">{packages.slice(0,6).map((pkg, i)=><Reveal delayMs={i*90} key={pkg.id} className="home-package-reveal"><Link to={`/tour-packages/${pkg.id}`} className="home-package-card"><div className="home-package-image">{pkg.imageUrl ? <img src={pkg.imageUrl} alt={pkg.title} loading="lazy"/> : <div>No package image</div>}<span>{pkg.durationDays} Days</span></div><div className="home-package-body"><small>{pkg.destination}</small><h3>{pkg.title}</h3><p>{pkg.description || ""}</p><b>Explore Package <Icon name="arrowRight" size={14}/></b></div></Link></Reveal>)}</div>
        </div>
      </div></section>}
